...
def send_to_influxdb(meter_name, meter_type, voltage=None, current=None, power=None, latitude=None, longitude=None):
    ...
    try:
        ...
        print(f"✓ Data sent to InfluxDB: {meter_name} = {' | '.join(fields)}")
    except Exception as e:
        print(f"✗ InfluxDB Error: {e} (continuing)")
...
try:
    while True:
        ...
        # 1. GPS UPDATE (Every 30 Minutes) - Non-blocking
        ...
        for m in meters:
            ...
            while time.time() - meter_start < 5:
                try:
                    if m["type"] == "sdm120":
                        # SDM120 Register Addresses (Function Code 04)
